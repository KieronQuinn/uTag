...
import com.kieronquinn.app.utag.repositories.BaseSmartThingsServiceRepository.ServiceResponse
...
interface BaseSmartThingsServiceRepository<I: IInterface> {
...
abstract class BaseSmartThingsServiceRepositoryImpl<I: IInterface>(
    private val smartThingsRepository: SmartThingsRepository
): BaseSmartThingsServiceRepository<I>, KoinComponent {
    ...
    override suspend fun disconnect() {
        serviceConnection?.let {
            smartThingsRepository.unbindService(it)
        }
    }
        ...
        suspendCoroutine {
            ...
            val serviceConnection = object: IServiceConnection.Stub() {
                override fun onServiceConnected(
                    ...
                    scope.launch {
                        this@BaseSmartThingsServiceRepositoryImpl.service.emit(service)
                        if(!hasResumed){
                            hasResumed = true
                            try {
                                it.resume(service)
            ...
            try {
                if(!smartThingsRepository.bindService(serviceIntent, serviceConnection)) {
                    it.resume(null)
                }
