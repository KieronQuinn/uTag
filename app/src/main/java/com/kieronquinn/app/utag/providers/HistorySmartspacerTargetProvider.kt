...
class HistorySmartspacerTargetProvider: SmartspacerTargetProvider() {
    ...
    private val configIcon by lazy {
        AndroidIcon.createWithResource(provideContext(), R.drawable.ic_location_history)
    }
            ...
            if(it.image != null) {
                TargetTemplate.Image(
                    ...
                    componentName = ComponentName(
                        provideContext(), HistorySmartspacerTargetProvider::class.java
                    ),
                    icon = Icon(icon),
                    title = Text(it.title),
    ...
    override fun getConfig(smartspacerId: String?): Config {
        ...
        return Config(
            resources.getString(R.string.smartspacer_plugin_history_title),
            if(label != null) {
                resources.getString(R.string.smartspacer_plugin_history_description_set, label)
            }else{
                resources.getString(R.string.smartspacer_plugin_history_description)
            },
            if(smartspacerId == null) configIcon else icon,
            allowAddingMoreThanOnce = true,
            widgetProvider = HistorySmartspacerWidgetProvider.AUTHORITY
        )
    }
