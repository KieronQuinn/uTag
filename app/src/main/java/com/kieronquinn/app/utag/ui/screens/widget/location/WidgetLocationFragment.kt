...
import com.kieronquinn.app.utag.ui.screens.widget.location.WidgetLocationViewModel.Event
import com.kieronquinn.app.utag.ui.screens.widget.location.WidgetLocationViewModel.PreviewState
import com.kieronquinn.app.utag.ui.screens.widget.location.WidgetLocationViewModel.State
import com.kieronquinn.app.utag.ui.screens.widget.picker.WidgetDevicePickerFragment.Companion.setupMultipleTagPickerListener
...
class WidgetLocationFragment: BaseSettingsFragment(), BackAvailable, ProvidesBack, ProvidesOverflow {

    companion object {
        const val KEY_TAG_PICKER = "tag_picker"
        const val KEY_ON_CLICK_PICKER = "on_click_picker"
        const val KEY_STATUS_PICKER = "status_picker"
    }

    private val viewModel by viewModel<WidgetLocationViewModel> {
    ...
    private fun setupListeners() {
        setupMultipleTagPickerListener(KEY_TAG_PICKER) {
            viewModel.onDevicesChanged(it.toSet())
        }
        setupSingleTagPickerListener(KEY_ON_CLICK_PICKER) {
            viewModel.onClickDeviceChanged(it)
        }
        setupSingleTagPickerListener(KEY_STATUS_PICKER) {
            viewModel.onStatusDeviceChanged(it)
        }
    }
    ...
    private fun handleContent(state: State.Loaded) = setPreferences {
        ...
        preferenceCategory("options") {
            ...
            preference {
                title = getString(R.string.widget_configuration_location_devices_title)
                summary = if(state.devices.isEmpty()) {
                    getString(R.string.widget_configuration_location_devices_content)
                }else{
                    state.devices.joinToString(", ") { it.label }
                }
                onClick { viewModel.onDevicesClicked() }
            }
            //Only show open option if more than one Tag is selected
            val openDevice = (state.onClick ?: state.devices.firstOrNull())
                ?.takeIf { state.devices.size > 1 }
            if(openDevice != null) {
                preference {
                    title = getString(R.string.widget_configuration_location_click_device_title)
                    summary = getString(
                        R.string.widget_configuration_location_click_device_content,
                        openDevice.label
                    )
                    onClick { viewModel.onClickDeviceClicked() }
                }
            }
            //Only show status option if more than one Tag is selected
            val statusDevice = (state.status ?: state.devices.firstOrNull())
                ?.takeIf { state.devices.size > 1 }
            if(statusDevice != null) {
                preference {
                    title = getString(R.string.widget_configuration_location_status_device_title)
                    summary = getString(
                        R.string.widget_configuration_location_status_device_content,
                        statusDevice.label
                    )
                    onClick { viewModel.onStatusDeviceClicked() }
                }
            }
            dropDownPreference {
            ...
            dropDownPreference {
                ...
                isEnabled = state.mapStyle == MapStyle.NORMAL && state.devices.isNotEmpty()
                onChange<String> {
