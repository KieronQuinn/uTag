...
class HistorySmartspacerWidgetProvider: SmartspacerWidgetProvider() {

    companion object {
        internal const val AUTHORITY = "${BuildConfig.APPLICATION_ID}.widget.history"

        internal fun getProviderInfo(context: Context): AppWidgetProviderInfo {
            val appWidgetManager = AppWidgetManager.getInstance(context)
            val component = ComponentName(context, UTagHistoryWidgetProvider::class.java)
    ...
    override fun onWidgetChanged(smartspacerId: String, remoteViews: RemoteViews?) {
        ...
        smartspacerRepository.setTargetData(smartspacerId, targetData, true)
    }
